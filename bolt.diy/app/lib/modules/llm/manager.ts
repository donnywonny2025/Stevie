...
export class LLMManager {
  ...
  getDefaultProvider(): BaseProvider {
    // Use Google/Gemini as primary provider (confirmed working with unlimited quota)
    const googleProvider = this._providers.get('Google');
    if (googleProvider) {
      return googleProvider;
    }
    
    // Fallback to OpenRouter if Google is not available
    const openRouterProvider = this._providers.get('OpenRouter');
    if (openRouterProvider) {
      return openRouterProvider;
    }
    
    // Azure OpenAI as final fallback (though currently has deployment issues)
    const azureProvider = this._providers.get('Azure OpenAI');
    if (azureProvider) {
